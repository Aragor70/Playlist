...
describe('Test PUT /api/playlists/:id', () => {
    
    this.playlists = [{title: "title1"}, {title: "title2"}, {title: "title3"}, ];
    ...
    before((done) => {
        ...
        for (let i = 0; i < this.playlists.length; i++) {
            request(server.playlists).post('/api/playlists/')
                .send({ title: this.playlists[i].title })
                .catch((err) => done(err));
        }
        done();
    ...
    it ('For Fail, Return err msg when req title contains wrong characters.', (done) => {
        ...
        request(server.playlists).put('/api/playlists/2')
            .send({ title: '#error//' })
            .then((response) => {
                ...
                if(response.statusCode === 422) {
                    ...
                    expect(body.message).to.equal("playlist does not exist.")
                }
    ...
    it ('For Fail, Return 404 when params is not defined.', (done) => {
        ...
        request(server.playlists).put('/api/playlists/')
            .send({ title: 'name1' })
            .then((response) => {
    ...
    it ('For Success, Update last name.', (done) => {
        ...
        request(server.playlists).put('/api/playlists/' + 1)
            .send({ title: 'Ed' })
            .then((response) => {
                ...
                expect(body.message).to.equal("Playlist updated")
                expect(body.playlist.title).to.equal("Ed")
